...
public class ProductDAOImpl implements ProductDAO {
	...
	private static final String GET_PRODUCT_DETAILS = "SELECT PRODUCT_ID, P_NAME, P_PRICE, p_warranty_months, p_colour, p_deliverycharge, p_size, description, specification, keyfeatures FROM product, description  where product_id = ? AND product.d_id = description.description_id";

	private static final String GET_PRODUCT_DETAILS_BY_NAME = "SELECT PRODUCT_ID, P_NAME, P_PRICE, p_warranty_months, p_colour, p_deliverycharge, p_size, description, specification, keyfeatures FROM product, description  where p_name = ? AND product.d_id = description.description_id";
	...
	public ProductInfo getProduct(Connection connection, int productId) {
		...
		try {
			...
			productInfo = new ProductInfo(rs.getInt("product_id"),rs.getString("p_name"),rs.getInt("p_price"), rs.getInt("p_warranty_months"),
					                      ...
					                      rs.getString("description"), rs.getString("keyfeatures"),rs.getString("specification"));
